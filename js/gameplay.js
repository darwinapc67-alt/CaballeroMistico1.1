...
function restoreCheckpoint(preserveInfiniteProgress) {
    ...
    unlockedWeapons = Array.isArray(cp.unlockedWeapons) ? cp.unlockedWeapons.filter(function(id) { return WEAPON_CONFIG[id]; }) : [DEFAULT_WEAPON_ID];
  ...
  infiniteState = {
    ...
    weaponId: infiniteState.weaponId,
    unlockedWeapons: infiniteState.unlockedWeapons,
    hasBow: infiniteState.hasBow,
    arrows: infiniteState.arrows,
    bombs: infiniteState.bombs,
    ...
    if (infiniteState) {
      ...
      hasSword = infiniteState.hasSword;
      swordEquipped = infiniteState.swordEquipped;
      weaponId = normalizeWeaponId(infiniteState.weaponId);
      unlockedWeapons = infiniteState.unlockedWeapons || [DEFAULT_WEAPON_ID];
      if (unlockedWeapons.indexOf(DEFAULT_WEAPON_ID) < 0) unlockedWeapons.unshift(DEFAULT_WEAPON_ID);
      weaponId = isWeaponUnlocked(weaponId) ? weaponId : DEFAULT_WEAPON_ID;
      hasBow = infiniteState.hasBow;
      arrows = infiniteState.arrows;
...
function executeAdminCommand(rawCommand) {
  ...
  if (parts.join(" ") === "/give luz infinito" || parts.join(" ") === "/give luz inf" ||
  ...
  if (parts[0] === "/give") {
    ...
    if (item === "azari" && Number.isFinite(amount)) {
      ...
      if (parts[2] === "infinito" || parts[2] === "inf") {
        hasLantern = true;
        infiniteLight = true;
        adminCommandMessage = "Nivel de luz infinito activado.";
      } else {
    ...
    } else if (item === "luz" || item === "luzinfinita" || item === "infinite_light") {
      if (parts[2] === "infinito" || parts[2] === "inf") {
        hasLantern = true;
        infiniteLight = true;
        adminCommandMessage = "Nivel de luz infinito activado.";
...
function updateInfiniteMode() {
  ...
  if (infiniteSpawnTimer <= 0 && living === 0) {
    ...
    var previousWeaponCount = unlockedWeapons.length;
    unlockWeaponsForInfiniteWave(infiniteWave);
    if (unlockedWeapons.length > previousWeaponCount) {
      var infiniteWeapon = getWeaponConfig(unlockedWeapons[unlockedWeapons.length - 1]);
      weaponId = infiniteWeapon.id;
      player.weaponId = weaponId;
      player2.weaponId = weaponId;
      swordEquipped = true;
      spawnFloatText(player.x - 20, player.y - 58, infiniteWeapon.name + " desbloqueada", infiniteWeapon.color);
...
function explodeBomb(bomb) {
  ...
  enemies.forEach(function(enemy) {
    ...
    var dx = enemy.x + enemy.w / 2 - bomb.x;
    var dy = enemy.y + enemy.h / 2 - bomb.y;
  ...
  impactBursts.push({ x: bomb.x, y: bomb.y, life: 18, maxLife: 18, critical: true });
  sfxHit();
}

function updateBombs() {
  for (var i = bombsInFlight.length - 1; i >= 0; i--) {
    var bomb = bombsInFlight[i];
    var room = rooms[currentRoom];
    var previousBottom = bomb.y + bomb.h;
    bomb.vy += 0.35;
    bomb.x += bomb.vx;
    bomb.y += bomb.vy;
    bomb.vx *= 0.99;
    ...
    if (bomb.y >= landingY) {
      ...
      bomb.vy *= -0.35;
      bomb.vx *= 0.8;
      if (Math.abs(bomb.vy) < 0.8) bomb.life = Math.min(bomb.life, 8);
    }
    if (bomb.life <= 0 || bomb.x < 0 || bomb.x > WORLD_W) {
      explodeBomb(bomb);
      bombsInFlight.splice(i, 1);
    }
  }
}
...
function defeatBoss(e) {
  ...
  if (unlockWeaponAt(bossWeaponIndex)) {
    ...
    spawnFloatText(e.x - 25, e.y - 42, unlockedWeapon.name + " desbloqueada", unlockedWeapon.color);
    if (activeSlot >= 0) saveGame(activeSlot);
  }
...
function updateGenericPlayer(p, moveLeft, moveRight, jumpPressed, attackPressed, interactPressed, shootPressed, blockPressed, dashPressed, downPressed, bombPressed) {
  ...
  if (attackPressed && p.hasSword && !p.frozen && !p.blocking) {
    ...
    if (p.swordCooldown <= 0 && p.swordSwing <= 0 && !p.frozen && !p.blocking) {
      ...
      p.swordSwing = p.attackType === "charged" ? weapon.swing + 6 : weapon.swing;
  ...
  if (bombPressed && p.id === 1 && bombs > 0 && !p.frozen) {
    ...
    bombsInFlight.push({
      ...
      vx: p.facing * 6.5, vy: -7.5, life: 52
    });
    spawnParticles(p.x + p.w / 2, p.y + 14, "#ff7138", 5, 2);
    sfxBow();
  }
...
function applyWeaponEffect(p, enemy, weapon) {
  if (!weapon || !enemy) return;
  var x = enemy.x + enemy.w / 2, y = enemy.y + enemy.h / 2;
  if (weapon.effect === "shadow") {
    spawnParticles(x, y, "#8d5bd1", 5, 3);
  } else if (weapon.effect === "pierce") {
    p.vx = p.facing * 2;
    spawnParticles(x, y, "#73d4cc", 4, 2);
  } else if (weapon.effect === "stun") {
    enemy.attackTimer = Math.max(enemy.attackTimer || 0, 18);
    enemy.vx = 0;
    spawnFloatText(x, enemy.y - 20, "¡ATURDIDO!", "#f0a05a");
  } else if (weapon.effect === "pull") {
    enemy.vx = p.x < enemy.x ? -2 : 2;
    spawnParticles(x, y, "#7d69c9", 6, 3);
  } else if (weapon.effect === "time") {
    combatHitStop = Math.max(combatHitStop, 5);
    spawnParticles(x, y, "#72d9ff", 5, 2);
  }
}
...
function tryInteractFor(p) {
  ...
  if (room.healingStone && room.healingStone.active) {
    ...
    if (Math.sqrt(dx*dx + dy*dy) < 80 && !healing && healingStoneCooldown <= 0) {
      saveHealingStoneCheckpoint();
      healingStoneCooldown = 60;
      if (p.hp < p.maxHp) {
...
function startRiseThroughTransition(toRoom) {
  if (gameState === ST_TRANSITION) return;
  gameState = ST_TRANSITION;
  transPhase = "out";
  transTimer = 35;
  ...
  if (transPhase === "out") {
    ...
    if (transTimer === 25) {
      ...
      if (transIsFall) {
        ...
        if (twoPlayerMode) { player2.x = fallOrigin + ROOM_W / 2 - player2.w / 2 + 20; player2.y = 80; player2.vx = 0; player2.vy = 2; }
    ...
    if (transTimer <= 0) {
      ...
      if (transIsFall || transIsRise) {
        var cameraOrigin = rooms[currentRoom].worldX !== undefined ? rooms[currentRoom].worldX : currentRoom * ROOM_W;
        cameraX = Math.max(0, Math.min(cameraOrigin, WORLD_W - canvas.width));
        targetCamX = cameraX;
      }
      transIsFall = false; transIsRise = false; transitionCooldown = 45;
      startBossDialogue(currentRoom);
    }
  }
}
