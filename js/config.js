...
var ST_LANGUAGE = 0, ST_DEVICE = 1, ST_MENU = 2, ST_PLAYING = 3, ST_PAUSED = 4, ST_TRANSITION = 5, ST_EXPLOSION = 6, ST_INVENTORY = 7, ST_EDITOR = 8, ST_EDITOR_MENU = 9;
...
// ============================================
// SISTEMA DE PUBLICACIÓN Y EDITOR DE NIVELES
// ============================================
...
function getRoomName(index) {
    ...
    return names[index] || "Sala " + (index + 1);
}

var editorMode = false;
var editorSelection = 0; // 0: plataforma, 1: púa, 2: enemigo, 3: borrar
var editorElements = [];
var editorMouseX = 0, editorMouseY = 0;
var editorMenuSelection = 0;
var savedLevels = [];

// Estado avanzado del editor
var editorSelectedElement = -1;
var editorDragging = false;
var editorResizing = false;
var editorResizeHandle = ''; // 'se' = esquina inferior derecha
var editorGridSize = 10;
var editorShowGrid = true;
var editorSnapToGrid = true;

function loadSavedLevels() {
    try {
        var data = localStorage.getItem('savedLevels');
        if (data) {
            savedLevels = JSON.parse(data);
        } else {
            savedLevels = [];
        }
    } catch(e) {
...
var LevelPublisher = {
    shareViaURL: function(roomIndex) {
        var name = getRoomName(roomIndex) || "Sala " + (roomIndex + 1);
        alert("🔗 Compartir URL - Nivel: " + name + "\n(Próximamente)");
    },
    saveToLibrary: function(roomIndex, name) {
        alert("💾 Guardar en biblioteca - Nivel: " + name + "\n(Próximamente)");
    },
    publishToGist: function(roomIndex, name) {
        alert("🐙 Publicar en Gist - Nivel: " + name + "\n(Próximamente)");
    },
    loadFromLibrary: function() {
        alert("📂 Cargar de biblioteca\n(Próximamente)");
    },
...
function toggleEditor() {
    ...
    if (editorMode) {
        gameState = ST_EDITOR;
        editorElements = [];
        spawnFloatText(400, 300, "🎨 EDITOR DE NIVELES", "#0ff");
        spawnFloatText(400, 320, "1:Plataforma  2:Púa  3:Enemigo  X:Borrar  S:Guardar  ESC:Salir", "#aaa");
    } else {
...
function saveLevelFromEditor() {
    if (editorElements.length === 0) {
        spawnFloatText(400, 300, "No hay elementos para guardar", "#f44");
        return;
    }
    
    var name = prompt("Nombre del nivel:", "Mi nivel " + (savedLevels.length + 1));
    if (!name) return;
    ...
    editorElements.forEach(function(el) {
        if (el.type === 'platform') {
            newRoom.platforms.push({x: el.x, y: el.y, w: el.w, h: el.h});
        } else if (el.type === 'spike') {
            newRoom.spikes.push({x: el.x, y: el.y, w: el.w, h: el.h});
        } else if (el.type === 'enemy') {
            enemies.push({
                x: el.x, y: el.y, w: 24, h: 20, 
                vx: 1.5, vy: 0, baseY: el.y, range: 60, 
                dead: false, room: currentRoom, type: 'bat'
            });
        }
    });
    ...
    var levelData = {
        ...
        name: name,
        room: newRoom,
        enemies: editorElements.filter(function(el) { return el.type === 'enemy'; }).map(function(el) {
            return {x: el.x, y: el.y, w: 24, h: 20, type: 'bat', baseY: el.y, range: 60};
        }),
    ...
    spawnFloatText(400, 300, "✅ Nivel guardado: " + name, "#0f0");
    editorMode = false;
    gameState = ST_PLAYING;
    spawnFloatText(400, 320, "Niveles guardados: " + savedLevels.length, "#0ff");
}

function loadLevelFromList(index) {
    if (index < 0 || index >= savedLevels.length) return false;
    var level = savedLevels[index];
    if (!level) return false;
    
    rooms[currentRoom] = level.room;
    
    if (level.enemies) {
        level.enemies.forEach(function(e) {
            enemies.push({
                x: e.x, y: e.y, w: e.w || 24, h: e.h || 20,
                vx: 1.5, vy: 0, baseY: e.baseY || e.y, range: e.range || 60,
                dead: false, room: currentRoom, type: e.type || 'bat'
            });
        });
    }
    
    spawnFloatText(400, 300, "📂 Nivel cargado: " + level.name, "#0f0");
    return true;
}

function deleteLevelFromList(index) {
    if (index < 0 || index >= savedLevels.length) return;
    var name = savedLevels[index].name;
    savedLevels.splice(index, 1);
    saveLevelsToStorage();
    spawnFloatText(400, 300, "🗑️ Nivel eliminado: " + name, "#f44");
}

function addEditorElement(x, y, type) {
    ...
    if (type === 'platform') {
        editorElements.push({type: 'platform', x: worldX, y: worldY, w: 60, h: 14});
        spawnFloatText(x, y - 20, "➕ Plataforma", "#0f0");
    } else if (type === 'spike') {
        editorElements.push({type: 'spike', x: worldX, y: worldY, w: 40, h: 20});
        spawnFloatText(x, y - 20, "➕ Púa", "#f44");
    } else if (type === 'enemy') {
        editorElements.push({type: 'enemy', x: worldX, y: worldY, w: 24, h: 20});
        spawnFloatText(x, y - 20, "➕ Enemigo", "#f0f");
    }
}

function removeEditorElement(x, y) {
    var worldX = x + cameraX;
    var worldY = y + cameraY;
    for (var i = editorElements.length - 1; i >= 0; i--) {
        var el = editorElements[i];
        if (worldX >= el.x && worldX <= el.x + el.w &&
            worldY >= el.y && worldY <= el.y + el.h) {
            editorElements.splice(i, 1);
            spawnFloatText(x, y - 20, "🗑️ Elemento borrado", "#f44");
            return;
        }
    }
    spawnFloatText(x, y - 20, "No hay elemento aquí", "#888");
}

function initLevelPublisher() {
    loadSavedLevels();
    console.log("📤 LevelPublisher: Inicializado - " + savedLevels.length + " niveles guardados");
}

// ============================================
// FUNCIONES MEJORADAS DEL EDITOR
// ============================================

function snapToGrid(value) {
    if (!editorSnapToGrid) return value;
    return Math.round(value / editorGridSize) * editorGridSize;
}

function getElementAtMouse(screenX, screenY) {
...
function getResizeHandleAtMouse(elementIndex, screenX, screenY) {
    if (elementIndex < 0) return '';
    var el = editorElements[elementIndex];
    var worldX = screenX + cameraX;
    var worldY = screenY + cameraY;
    var handleSize = 8;
    
    // Esquina inferior derecha
    if (worldX >= el.x + el.w - handleSize && worldX <= el.x + el.w &&
        worldY >= el.y + el.h - handleSize && worldY <= el.y + el.h) {
        return 'se';
    }
    return '';
}

function handleEditorMouseDown(screenX, screenY, button) {
    if (gameState !== ST_EDITOR) return;
    
    var elementIndex = getElementAtMouse(screenX, screenY);
    
    if (button === 0) { // Click izquierdo
        if (editorSelection === 3) { // Modo borrar
            if (elementIndex >= 0) {
                editorElements.splice(elementIndex, 1);
                spawnFloatText(screenX, screenY - 20, "🗑️ Elemento borrado", "#f44");
                editorSelectedElement = -1;
            }
        } else {
            // Agregar nuevo elemento o seleccionar existente
            if (elementIndex >= 0) {
                editorSelectedElement = elementIndex;
                editorDragging = true;
            } else {
                addEditorElement(screenX, screenY, 
                    editorSelection === 0 ? 'platform' :
                    editorSelection === 1 ? 'spike' :
                    editorSelection === 2 ? 'enemy' : 'platform');
                editorSelectedElement = editorElements.length - 1;
                editorDragging = true;
            }
        }
        ...
        if (elementIndex >= 0) {
            var el = editorElements[elementIndex];
            var resizeHandle = getResizeHandleAtMouse(elementIndex, screenX, screenY);
            if (resizeHandle) {
                editorResizing = true;
                editorResizeHandle = resizeHandle;
                editorSelectedElement = elementIndex;
            }
        }
    }
...
function handleEditorMouseMove(screenX, screenY) {
    if (gameState !== ST_EDITOR) return;
    
    editorMouseX = screenX;
    editorMouseY = screenY;
    
    if (editorDragging && editorSelectedElement >= 0) {
        var el = editorElements[editorSelectedElement];
        var worldX = screenX + cameraX;
        var worldY = screenY + cameraY;
        
        el.x = snapToGrid(worldX - el.w / 2);
        el.y = snapToGrid(worldY - el.h / 2);
    } else if (editorResizing && editorSelectedElement >= 0 && editorResizeHandle === 'se') {
        var el = editorElements[editorSelectedElement];
        var worldX = screenX + cameraX;
        var worldY = screenY + cameraY;
        
        el.w = Math.max(20, snapToGrid(worldX - el.x));
        el.h = Math.max(14, snapToGrid(worldY - el.y));
    }
}
...
function exportLevelAsJSON(roomIndex) {
    var room = rooms[roomIndex];
    var levelData = {
        roomIndex: roomIndex,
        roomName: getRoomName(roomIndex),
        height: room.height,
        platforms: room.platforms || [],
        spikes: room.spikes || [],
        decor: room.decor || [],
        enemies: enemies.filter(function(e) { return e.room === roomIndex && !e.dead; })
    };
    return JSON.stringify(levelData, null, 2);
}

function copyLevelToClipboard(roomIndex) {
    var json = exportLevelAsJSON(roomIndex);
    if (navigator.clipboard) {
        navigator.clipboard.writeText(json).then(function() {
            spawnFloatText(400, 300, "📋 Nivel copiado al portapapeles", "#0ff");
        });
    } else {
        alert("Contenido a copiar:\n\n" + json);
    }
}